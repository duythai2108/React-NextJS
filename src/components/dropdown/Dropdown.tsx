    ...
    <Menu as="div" className="relative basis-[160px]">
      {({ open }) => (
        <>
          <Menu.Button
            as="div"
            className="flex items-center justify-between p-2.5 rounded-lg bg-grayf7 gap-2.5 text-xs font-medium w-full"
          >
            <span>{selected}</span>
            <span
          ...
          <Menu.Items
            as="div"
            className="absolute top-full left-0 right-0 z-10 bg-grayfc rounded-lg shadow-dropdown outline-none"
          >
            {data.length > 0 &&
              data.map((item) => (
                <Menu.Item
